...
(function ($) {
    ...
    let _deleteItem = function (widgetOptions, e, $elem) {
        let count = _count($elem, widgetOptions);

        if (count > widgetOptions.min) {
            $todelete = $elem.closest(widgetOptions.widgetItem);

            // trigger a custom event for hooking
            let eventResult = $('.' + widgetOptions.widgetContainer).triggerHandler(events.beforeDelete, $todelete);
    ...
    let _getNewIdValues = function (id, index) {
        ...
        return [newID, widgetsOptions];
    };

    let _updateAttrID = function ($elem, index) {
        let widgetOptions = eval($elem.closest('div[data-dynamicform]').attr('data-dynamicform'));
        let id = $elem.attr('id');
        let newID = id;

        if (id !== undefined) {
            let newIdValues = _getNewIdValues(id, index);
            newID = newIdValues[0];
            if (newIdValues[1] !== undefined) {
                widgetOptions = newIdValues[1];
            }
    ...
    let _updateAttrName = function ($elem, index) {
    ...
    let _updateDepDropDowmChildSelectors = function ($elem, index) {
        let childSelectors = $elem.data('child-selectors');
        let newChildSelectors = [];
        if (childSelectors !== undefined) {
            for (let i in childSelectors) {
                let selector = childSelectors[i];
                if ($(selector).length) {
                    //get new id given old
                    let id = selector.substr(1);
                    let newIdValues = _getNewIdValues(id, index);
                    let newID = newIdValues[0];
                    newChildSelectors.push('#' + newID);
                }
            }
            $elem.data('child-selectors', newChildSelectors);
        }
    };
    ...
    var _updateAttributes = function (widgetOptions) {
        ...
        $(widgetOptionsRoot.widgetItem).each(function (index) {
            ...
            $(this).find('input,select,textarea').each(function () {
                ...
                _updateDepDropDowmChildSelectors($(this), index);
            });
        });
    };
